    ...
    CheckpointObject* createCheckpoint() {
        ...
        if (shouldSave) {
            ...
            if (m_checkpointArray) {
                m_fields->m_checkpoints.erase(
                    std::remove_if(m_fields->m_checkpoints.begin(), m_fields->m_checkpoints.end(),
                        [this](PracticeCheckpointData const& stored) {
                            return !m_checkpointArray->containsObject(stored.checkpoint);
                        }),
                    m_fields->m_checkpoints.end());

                for (auto it = bot.checkpoints.begin(); it != bot.checkpoints.end();) {
                    if (!m_checkpointArray->containsObject(it->first))
                        it = bot.checkpoints.erase(it);
                    else
                        ++it;
                }
            }
